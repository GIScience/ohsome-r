...
This ohsome R package grants access to the power of the [ohsome](https://heigit.org/de/big-spatial-data-analytics/ohsome-3/){target=blank}
...
- retrieve the geometry of the historical OpenStreetMap data, e.g., to visualize 
the evolution of certain OpenStreetMap elements over time. You can get the 
geometries for specific points in time or all changes within a timespan 
(full-history).
...
Here is how to create a query of the number of breweries at January 1st of each 
...
or `rename()` from the `dplyr` to create such a column as in the example above.
