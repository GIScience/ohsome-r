...
Heidelberg main station with their tags. The response is used to visualise
...
aggregation and extraction. However, the ohsome API also has endpoints for the 
aggregation of users and contributions as well as for the extraction of 
contributions and of the full history of elements.
...
City, filter for features that still exist and visualise all building features 
with their year of creation:

```{r buildings, message = FALSE}
...
As seen above, `sf` objects can be passed into the `boundary` argument of `ohsome_query()` and any of its wrapper functions. You can also update queries 
with `set_boundary()`. The `sf` object will be converted into GeoJSON and passed into the `bpolys` parameter 
of the query.
